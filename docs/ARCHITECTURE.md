...
![CCLRTE Network Topology — RPi5 with eth0 (CODESYS), wlan0 (management), eth1 (EtherCAT/Waveshare PCIe RTL8111H), SPI0 (IO-Link), UART0 (Modbus), SPI1 (CAN)](images/network-topology.svg)

**eth0** is the native RPi5 GbE (BCM2712 / RP1 MAC), static 192.168.2.100/24, no default gateway — CODESYS programming and OPC-UA. **eth1** is the **Waveshare PCIe TO Gigabit ETH Board (C)** (RTL8111H, PCIe x1 Gen2, HAT+ FPC connector), raw link with no IP assigned — dedicated to fieldbus. `protocol-manager.sh` enforces mutual exclusivity: only one of EtherCAT / PROFINET / Modbus-TCP may be active on eth1 at a time.
...
![CPU Layout — CPU0+CPU1 Linux domain vs CPU2+CPU3 real-time isolated domain with roles, priorities, and measured latency](images/cpu-layout.svg)
...
![RT Determinism Stack — five layers from Firmware through Kernel, CPU Freq, Scheduler, to Application with all tuning parameters](images/rt-stack.svg)
...
![Software Architecture — CODESYS IDE to KAS build system, showing all components, systemd services, kernel, and meta-cclrte layer](images/software-layers.svg)

---

## systemd Service Dependency Chain

![systemd Service Dependency Chain — RT boot path (rt-setup → xenomai-setup → ethercat → codesyscontrol) and parallel services](images/service-chain.svg)
